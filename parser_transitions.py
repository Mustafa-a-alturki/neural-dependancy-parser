...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         2. Left Arc
        ###         3. Right Arc
        last_stack_idx = len(self.stack) - 1
        if transition == 'S':
            self.stack.append(self.buffer.pop(0))
        elif transition == 'LA':
            self.dependencies.append((self.stack[last_stack_idx], self.stack.pop(last_stack_idx - 1)))
        elif transition == 'RA':
            self.dependencies.append((self.stack[last_stack_idx - 1], self.stack.pop(last_stack_idx)))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for transition, unfinished_parse in zip(transitions, minibatch_parses):
